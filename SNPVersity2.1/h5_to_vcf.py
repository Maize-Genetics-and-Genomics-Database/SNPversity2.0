...
if len(variant_data['POS']) == 0:
    ...
    with open(output_vcf_path, 'w') as vcf_file:
        ...
        if "maizegdb" in hdf5_file_path:
            ...
            vcf_file.write("##citation=Andorf CM, Ross-Ibarra J, Seetharam AS, Hufford MB, Woodhouse MR. (2024) A unified VCF data set from nearly 1,500 diverse maize accessions and resources to explore the genomic landscape of maize. G3 Genes|Genomes|Genetics.\n")
            vcf_file.write("##reference=B73_v5\n")
            ...
            vcf_file.write("##INFO=<ID=plantcad1_score,Number=.,Type=Float,Description=\"DNA-based language model PlantCaduceus1 score\">\n")
            vcf_file.write("##INFO=<ID=plantcad2_score,Number=.,Type=Float,Description=\"DNA-based language model PlantCaduceus2 score\">\n")
            vcf_file.write("##INFO=<ID=ESM1_score,Number=.,Type=Float,Description=\"Protein-based language model (ESM1b) score\">\n")
            vcf_file.write("##INFO=<ID=ESM2_score,Number=.,Type=Float,Description=\"Protein-based language model (ESM-2) score\">\n")
            vcf_file.write("##INFO=<ID=ESM3_score,Number=.,Type=Float,Description=\"Protein-based language model (ESM-3) score\">\n")
        ...
        if "nam" in hdf5_file_path:
            ...
            vcf_file.write("##doi=https://doi.org/10.1101/2024.04.30.591904\n")
            vcf_file.write("##citation=Hufford MB, Seetharam AS, Woodhouse MR, et al. De novo assembly, annotation, and comparative analysis of 26 diverse maize genomes. Science. 2021;373(6555):655-662.\n")
            vcf_file.write("##reference=B73_v5\n")
        ...
        if "schnable" in hdf5_file_path:
            ...
            vcf_file.write("##reference=Andorf CM, Ross-Ibarra J, Seetharam AS, Hufford MB, Woodhouse MR. (2024) A unified VCF data set from nearly 1,500 diverse maize accessions and resources to explore the genomic landscape of maize. G3 Genes|Genomes|Genetics.\n")
            vcf_file.write("##doi=https://doi.org/10.1101/2024.04.30.591904\n")
            vcf_file.write("##citation=Grzybowski MW, Mural RV, Xu G, Turkus J, Yang J, Schnable JC. A common resequencing-based genetic marker data set for global maize diversity. Plant J. 2023;113(6):1109-1121.\n")
            vcf_file.write("##reference=B73_v5\n")
            ...
            vcf_file.write("##FORMAT=<ID=GT,Number=1,Type=String,Description=\"Genotype\">\n")
            vcf_file.write("#" + "\t".join(["CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT"] + genome_list) + "\n")
        if "panzea_hapmap" in hdf5_file_path:
            vcf_file.write("##fileformat=VCFv4.2\n")
            vcf_file.write("##fileDate=" + current_date + "\n")
            vcf_file.write("##source=PANZEA_Hapmap_2016\n")
            vcf_file.write("##citation=Bukowski et al (2017) Construction of the third-generation Zea mays haplotype map. GigaScience.\n")
            vcf_file.write("##reference=B73_v5\n")
            vcf_file.write("##doi=https://doi.org/10.1093/gigascience/gix134\n")
            vcf_file.write("##INFO=<ID=MQ,Number=1,Type=Float,Description=\"RMS mapping quality\">\n")
            vcf_file.write("##INFO=<ID=CVC,Number=1,Type=Integer,Description=\"The number of accessions that have genotype data for a particular variant\">\n")
            vcf_file.write("##INFO=<ID=CVP,Number=1,Type=Float,Description=\"The percent of accessions that have genotype data for a particular variant.\">\n")
            vcf_file.write("##INFO=<ID=TYPE,Number=.,Type=String,Description=\"The type of effect using Sequence Ontology terms\">\n")
